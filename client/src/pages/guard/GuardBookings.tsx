...
export function GuardBookings() {
  ...
  return (
    <>
      <PageHeader
        title="My bookings"
        description="Confirm jobs, mark them complete and read customer reviews."
      />
      <Tabs
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Accepted requests become bookings — they'll appear in this list."
        />
      ) : (
        <div className="space-y-4">
          {filtered.map((booking) => (
            <BookingCard key={booking.id} booking={booking} perspective="guard" onChanged={refetch} />
          ))}
        </div>
      )}
    </>
